...
|:-----|----------:|------------:|-----------:|-------:|------------------:|
    ...
    for row in activity:
        ...
        if row["ult_comentario"]:
            ...
            row["ult_comentario"] = ""
        f.write('''
| [{site}](https://{site}) | [{num}]({admin}edit.php?orderby=date&order=desc) | [{comentarios}]({admin}edit-comments.php?comment_type=comment&orderby=comment_date&order=desc) | {fin:%Y-%m-%d} | {ini:%Y-%m-%d} | {ult_comentario} |
        '''.rstrip().format(**row))
    f.write('''\n
